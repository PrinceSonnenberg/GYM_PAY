...
const templates = ['Weight Loss', 'Hypertrophy', 'Endurance', 'Flexibility'];
...
const ClientGoalsPage: React.FC = () => {
    ...
    const { clients, getClientGoals, addGoal } = useData();
    ...
    const commitDraft = (): boolean => {
        ...
        const goal: Omit<ActiveGoal, 'id'> = {
            ...
            description: draft.date ? `${draft.metric} • Ends ${draft.date}` : draft.metric,
            progress: 0,
                ...
                <div className="flex flex-col gap-3 my-4">
                    ...
                    <div className="flex gap-3 px-4 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                        {templates.map(template => (
                            <button key={template} onClick={() => applyTemplate(template)} className={`flex h-9 shrink-0 items-center justify-center gap-x-2 rounded-full bg-white border-2 px-4 active:scale-95 transition-transform ${draft.title === template ? 'border-primary bg-primary-soft' : 'border-ink'}`}>
                                <Icon name={templateIcons[template]} className="text-primary text-[18px]" />
                ...
                {goals.length > 0 && (
                    <div className="flex flex-col gap-4 mt-4">
                        ...
                        <div className="flex flex-col gap-3 px-4">
                            {goals.map((goal) => (
                                <div key={goal.id} className="bg-white rounded-2xl p-4 border-2 border-ink">
                                    <div className="flex justify-between items-start mb-3">
                                        <div className="flex items-center gap-3">
                                            <div className={`size-10 rounded-lg flex items-center justify-center ${goal.iconBg} ${goal.iconColor}`}><Icon name={goal.icon} /></div>
                                            <div>
                                    ...
                                    <div className="flex items-end justify-between mb-2">
                                        <div className="flex flex-col">
                                            ...
                                            <div className="flex items-baseline gap-1">
                                                <span className="text-xl font-bold font-mono">{goal.currentValue}</span>
                                                <span className="text-sm text-text-muted font-mono">/ {goal.targetValue}</span>
                                            </div>
                                        </div>
                                        <span className={`text-sm font-bold px-2 py-1 rounded-full ${goal.iconBg} ${goal.iconColor}`}>{goal.progressLabel}</span>
                                    </div>
                                    <div className="w-full bg-background h-2 rounded-full overflow-hidden border border-border-light">
                                        <div className={`${goal.progressColor} h-full rounded-full`} style={{ width: `${goal.progress}%` }}></div>
                                    </div>
                                </div>
                            ))}
                        </div>
